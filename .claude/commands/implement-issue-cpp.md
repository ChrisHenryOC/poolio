---
allowed-tools: Bash(gh issue view:*),Bash(gh issue list:*),Bash(git checkout:*),Bash(git branch:*),Bash(git push:*),Bash(pio run:*),Bash(pio test:*)
description: Analyze and fix a GitHub issue (C++ / PlatformIO)
---

Analyze and fix GitHub issue: $ARGUMENTS (C++ implementation)
...
1. **Red**: Write a failing test that defines the expected behavior
   ...
   ```bash
   pio test -e native  # Should fail
   ```

2. **Green**: Write the minimum code to make the test pass

   ```bash
   pio test -e native  # Should pass
   ```
...
3. **Refactor**: Clean up while keeping tests green
   ...
   ```bash
   pio test -e native  # Still passes
   ```
...
```bash
# Build succeeds (nonprod environment)
pio run -e nonprod
...
**Note:** Unlike Python, there's no separate linting/formatting step (clang-format skipped for now).
...
Note: Code review happens via `/review-pr-cpp` after PR creation.
