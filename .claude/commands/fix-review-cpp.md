---
allowed-tools: Bash(gh pr comment:*),Bash(gh pr diff:*),Bash(gh pr view:*),Bash(gh api:*),Bash(git add:*),Bash(git commit:*),Bash(git push:*),Bash(pio run:*),Bash(pio test:*)
description: Fix high and medium severity issues from C++ code review for PR $ARGUMENTS.
---

Fix high and medium severity issues from C++ code review for PR $ARGUMENTS.
...
# Verify the diff file exists (run /review-pr-cpp first if not)
test -f code_reviews/PR$ARGUMENTS-*/pr.diff || echo "ERROR: Diff not found. Run /review-pr-cpp $ARGUMENTS first."
# Verify we're on the correct branch
gh pr view $ARGUMENTS --json headRefName -q '.headRefName'
git fetch origin <branch> && git checkout <branch>
```

The diff is already saved at `code_reviews/PR$ARGUMENTS-<title>/pr.diff` from `/review-pr-cpp`. If the diff file doesn't exist, run `/review-pr-cpp $ARGUMENTS` first.
...
- One todo per issue with severity prefix (e.g., "[High] Fix buffer overflow")
